...
from src.app.settings import settings, RepositoryType
...
def create_product_repository() -> IProductRepository:
    ...
    if _product_repository is None:
        if settings.repository_type == RepositoryType.IN_MEMORY:
            from src.app.repositories.in_memory.product_repository import InMemoryProductRepository
            _product_repository = InMemoryProductRepository()
        elif settings.repository_type == RepositoryType.JSON:
            from src.app.repositories.json.product_repository import JSONProductRepository
...
def create_category_repository() -> ICategoryRepository:
    ...
    if _category_repository is None:
        if settings.repository_type == RepositoryType.IN_MEMORY:
            from src.app.repositories.in_memory.category_repository import InMemoryCategoryRepository
            _category_repository = InMemoryCategoryRepository()
        elif settings.repository_type == RepositoryType.JSON:
            from src.app.repositories.json.category_repository import JSONCategoryRepository
